...
def generate_launch_description():
    ...
    gazebo_models_subpath = 'models'
    default_world_file = 'empty.world'
    gazebo_worlds_subpath = 'worlds'
    ros_gz_bridge_config_subpath = 'config/ros_gz_bridge.yaml'
    rviz_config_filename = 'yahboom_rosmaster_gazebo_sim.rviz'

    # Dynamically find package share directories
    ...
    # Full paths constructed dynamically
    default_ros_gz_bridge_config_file_path = os.path.join(pkg_share_gazebo, ros_gz_bridge_config_subpath)
    default_rviz_config_path = PathJoinSubstitution([pkg_share_gazebo, 'rviz', rviz_config_filename])
    gazebo_models_path = os.path.join(pkg_share_gazebo, gazebo_models_subpath)
    ...
    world_path = PathJoinSubstitution([pkg_share_gazebo, gazebo_worlds_subpath, world_file])
    ...
    declare_x_cmd = DeclareLaunchArgument(name='x', default_value='0.0', description='x initial position, meters')
    declare_y_cmd = DeclareLaunchArgument(name='y', default_value='0.0', description='y initial position, meters')
    declare_z_cmd = DeclareLaunchArgument(name='z', default_value='0.05', description='z initial position, meters')
    declare_roll_cmd = DeclareLaunchArgument(name='roll', default_value='0.0', description='roll initial orientation, radians')
    declare_pitch_cmd = DeclareLaunchArgument(name='pitch', default_value='0.0', description='pitch initial orientation, radians')
    declare_yaw_cmd = DeclareLaunchArgument(name='yaw', default_value='0.0', description='yaw initial orientation, radians')
    ...
    # Set Gazebo model path environment variables (both GAZEBO_MODEL_PATH and GZ_SIM_RESOURCE_PATH)
    ...
    # Image bridge optimization node
    ...
    # Add environment variable actions for Gazebo model and resource paths
    ld.add_action(set_env_vars_models)
    ld.add_action(set_env_vars_resources)

    # Add other launch actions

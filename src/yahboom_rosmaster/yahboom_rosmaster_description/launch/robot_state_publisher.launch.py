...
def process_ros2_controllers_config(context):
    ...
    # Use dynamic package share discovery instead of hardcoded home path
    pkg_share_description = FindPackageShare(package='yahboom_rosmaster_description').perform(context)

    # Source config path (within package)
    src_config_path = os.path.join(pkg_share_description, 'config', robot_name)

    # Install config path (same here since we use package share)
    install_config_path = os.path.join(pkg_share_description, 'config', robot_name)
